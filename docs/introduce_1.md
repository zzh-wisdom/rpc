...
- [3. 特性](#3-特性)
- [4. RPC 结构](#4-rpc-结构)
- [5. RPC的消息传递](#5-rpc的消息传递)
  - [5.1. 消息传递的步骤](#51-消息传递的步骤)
  - [5.2. 数据传输协议](#52-数据传输协议)
- [6. 一个简单实现例子](#6-一个简单实现例子)
- [7. RPC 类型](#7-rpc-类型)
- [8. 优点](#8-优点)
- [9. 缺点](#9-缺点)
- [10. 参考文献](#10-参考文献)

基础篇主要介绍RPC的基础知识，包括基本概念、出现背景、简单的原理（RPC特性、结构、消息传递机制等等）、RPC的类型以及优缺点。
...
RPC将通信过程中涉及的很多问题进行了封装，使得应用开发更加简单和高效。
...
1. 简化分布式计算。开发人员不用关注任何通信细节，只用考虑分布式系统的核心问题：计时、各独立组件的故障和独行运行逻辑的划分。
2. RPC通信更加高效。
3. RPC提供的语义尽量强大。
4. RPC的安全通信。
...
1. 使用**客户端-服务器**的模型交互（调用者是客户端，执行者是服务器）
